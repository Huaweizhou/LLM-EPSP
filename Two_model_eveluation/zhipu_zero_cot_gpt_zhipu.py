...
api相关的属性
'''
client_2 = OpenAI(api_key="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")
MODEL_2 = "gpt-4o-mini-2024-07-18"

client_1 = ZhipuAI(api_key = "5f12e86015e0a1cfe113f50960bfcf0c.T6iNWxjOaztQXpeQ")
MODEL_1 = "glm-4-0520"
...
for testcase in x_test_instance:
    ...
    """
    # 证明者生成回答（glm-4-0520）
    response_1 = client_1.chat.completions.create(
        model=MODEL_1,
        messages = [{"role":"system","content":system_message}] + [{"role": "user","content":question_prompt}],
        stream = False,
        temperature=0,
    )
    response_glm_4_max = response_1.choices[0].message.content
    print("response_glm_4_max:", response_glm_4_max)

    # 验证者给出验证（glm-4-0520）
    response_2 = client_2.chat.completions.create(
            model=MODEL_2,
            messages = [{"role":"system","content":f"{system_message}Below you will get a judgment on whether a certain student can pass the exam. Please give your opinion based on the existing information."}] + [{"role": "user","content":f"You have ample time to think. Adhere to the above principles, please give your evaluation of this student's {testcase} judgment {response_glm_4_max}. Let's work this out in a step by step way to be sure we have the right answer. Do not provide any other words and analysis.just 'yes' or 'no'(i.e.,Options[yes or no])"}],
            stream = False,
            temperature=0,
        )
    # response_advice = response_2.choices[0].message.content
    # print("response_advice:",response_advice)
    
    # 证明者再次给出回答(glm-4-0520)
    # response_3 = client_1.chat.completions.create(
    #     model=MODEL_1,
    #     messages = [{"role":"system","content":system_message}] + [{"role": "user","content":f"You have ample time to think. Adhere to the above principles, please revise your previous answer {response_glm_4_max} based on the evaluation opinions {response_advice}. Let's work this out in a step by step way to be sure we have the right answer.Do not provide any other words and analysis.just 'yes' or 'no'."}],
    #     stream = False,
    #     temperature=0,
    # )
    response_final = response_2.choices[0].message.content
    response_final_list.append(response_final)
    print("response_final:", response_final)
    print("response_fianl_list:", response_final_list)

# 数据可视化
y_pred = response_final_list
